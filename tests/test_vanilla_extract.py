...
from django.forms import BaseForm, Form, ModelForm, fields
...
class BaseTestCase(TestCase):
    ...
    def assertContext(self, response, expected):
        ...
        for key in expected.keys():
            self.assertTrue(
                key in response.context_data, "context missing key '%s'" % key
            )
        for key in response.context_data.keys():
            self.assertTrue(
                key in expected, "context contains unexpected key '%s'" % key
            )
